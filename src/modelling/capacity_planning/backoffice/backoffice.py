...
class BackOfficeCalculator:
    ...
    def get_required_agents(self, lambdas: list, ahts: list, backlog_sum: Union[int, float], backlog_within: int,
                            occupancy: float):
        """
    ...
    def get_possible_volume(self, available_agents: list, ahts: list, backlog_within: int, occupancy: float):
        assert backlog_within < len(ahts)
        assert 0 < occupancy <= 1
        return ((np.array(available_agents) * occupancy) / (np.array(ahts))).tolist()
