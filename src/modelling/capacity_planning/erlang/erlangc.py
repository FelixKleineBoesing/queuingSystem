...
class ErlangC(Optimizer):
    ...
    def get_average_waiting_time_for_chat(self, lambda_: float, mu: float, max_sessions: int, number_agents: int,
        ...
        """
        aht = 1 / lambda_
        kwargs = {"mu": mu,  "lambda_": 1 / (aht * share_sequential_work * min(1, (max_sessions - 1))),
...
if __name__ == "__main__":
    ...
    erlang.plot_cost_function(erlang.get_average_waiting_time_for_chat,
                              kwargs={'lambda_': 0.05555555555555555, 'mu': 0.005555555555555556,
                                      'share_sequential_work': 0.15, 'max_sessions': 5},
                              target_value=50, boundaries=(1, 100), steps=1000, optim_argument="number_agents").show()